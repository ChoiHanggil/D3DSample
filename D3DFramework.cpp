...
LRESULT D3DFramework::MessgeHandle(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_PAINT:
...
{
	DXGI_SWAP_CHAIN_DESC scd; //DESC 설명한다
...
void D3DFramework::OnResize()
{
	...
	//RenderTarget
	mspSwapChain->GetBuffer(0, IID_PPV_ARGS(mspRenderTarget.ReleaseAndGetAddressOf())); //Textur 2D연결 
	...
	CD3D11_TEXTURE2D_DESC td(
		DXGI_FORMAT_D24_UNORM_S8_UINT,//깊이 24비트, 스텐실 8비트
		mScreenWidth,
		mScreenHeight,
		1,
		1,
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	return pFramework->MessgeHandle(hwnd, message, wParam, lParam);
}